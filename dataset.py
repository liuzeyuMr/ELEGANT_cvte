...
class MultiCelebADataset(object):
    def __init__(self, attributes, config=config):
        ...
        with open(os.path.join(self.config.data_dir, 'list_attr_celeba.txt'), 'r') as f:
            ...
            col_ids = [lines[1].split().index(attribute) + 1 for attribute in self.attributes] #找到属性列 假设是 7和8
            self.all_labels = np.array([[int(x.split()[col_id]) for col_id in col_ids] for x in lines[2:]], dtype=np.float32)#得到所有图像第7列和第8列的属性标签类似[0,1，1，0]
            self.im_names = np.array([os.path.join(self.config.data_dir, 'align_5p/{:06d}.jpg'.format(idx+1)) for idx in range(len(self.all_labels))]) #获得所有图片的名字

        self.dict = {i: {True: None, False: None} for i in range(len(self.attributes))}
        for attribute_id in range(len(self.attributes)):# 0 1
            for is_positive in [True, False]:
                idxs = np.where(self.all_labels[:,attribute_id] == (int(is_positive)*2 - 1))[0]
                im_names = self.im_names[idxs] #获得某一属性的一个数据集的全部名字 一个属性有两个数据集合
                labels = self.all_labels[idxs]
                data_gen = SingleCelebADataset(im_names, labels, self.config).gen()
                self.dict[attribute_id][is_positive] = data_gen#这里就是某一个属性，然后是否拥有这个属性代表的数据集合
